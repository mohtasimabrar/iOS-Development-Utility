...
open class CircularTrig {
    ...
    open class func drawUnfilledGradientArcInContext(_ ctx: CGContext, center: CGPoint, radius: CGFloat, lineWidth: CGFloat, minimumAngle: CGFloat, maximumAngle: CGFloat, colors: [UIColor], lineCap: CGLineCap) {
        // ensure two colors exist to create a gradient between
        guard colors.count == 2 else {
            return
        }
        
        let cartesianFromAngle = compassToCartesian(toRad(Double(minimumAngle)))
